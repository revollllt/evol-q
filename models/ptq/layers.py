...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self,
                 ...
                 groups=1,
                 bias=True,
                 quant=False,                                  # @ Victor: 是否进行量化
                 calibrate=False,                              # @ Victor: 是否进行校准
                 last_calibrate=False,                         # @ Victor: 是否是最后一次进行校准
                 bit_type=BIT_TYPE_DICT['int8'],               # @ Victor: 数据类型（见bit_type.py）
                 calibration_mode='layer_wise',                # @ Victor: 校准模式
                 observer_str='minmax',
    ...
    def forward(self, x):
        ...
        weight = self.quantizer(self.weight)                                # @ Victor: 经过了quantizer进行量化后的权重
...
class QAct(nn.Module):
    ...
    def forward(self, x, prev_layer=None, x_prev=None):
        ...
        if not self.quant:
            return x
        x = self.quantizer(x)                                # @ Victor: 经过了quantizer进行量化后的Activation
        return x
