...
# @ Victor: --------------------------------------------------------------------------------- # 
from .fastvit_mobileone_quant import MobileOneBlock, MobileOneBlock_Q           # ! Change this later
from .fastvit_replknet_quant import ReparamLargeKernelConv, ReparamLargeKernelConv_Q    # ! Change this later
...
def convolutional_stem_q(
    ...
    return nn.Sequential(
        MobileOneBlock_Q(                                    # ! Should be changed here
...
class MHSA(nn.Module):                                    # ! Should be changed here
...
class MHSA_Q(nn.Module):                                    # ! Should be changed here
    """Multi-headed Self Attention module.
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        block = list()
        block.append(
            ReparamLargeKernelConv(                                    # ! Should be changed here
                in_channels=in_channels,
...
class PatchEmbed_Q(nn.Module):
    ...
    def __init__(
        ...
        block = list()
        block.append(
            ReparamLargeKernelConv_Q(                                    # ! Should be changed here
                in_channels=in_channels,
                out_channels=embed_dim,
                kernel_size=patch_size,
                stride=stride,
...
class RepMixerBlock_Q(nn.Module):
    """Implementation of Metaformer block with RepMixer as token mixer.
...
class AttentionBlock_Q(nn.Module):
    """Implementation of metaformer block with MHSA as token mixer.
    ...
    def __init__(
        ...
        self.norm = norm_layer(dim)
        self.token_mixer = MHSA_Q(dim=dim, quant=quant, calibrate=calibrate, cfg=cfg)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.fork_feat:
            ...
            self.conv_exp = MobileOneBlock_Q(
...
def fastvit_sa12(pretrained=False, quant=False, calibrate=False, cfg=None, **kwargs):
    ...
    mlp_ratios = [4, 4, 4, 4]
    downsamples = [True, True, True, True]
    pos_embs = [None, None, None, partial(RepCPE_Q, spatial_shape=(7, 7))]
